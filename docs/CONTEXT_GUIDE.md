...
→ GENERAL CONTEXT — Voxy sees all your workspaces and Home cards, nothing specific.

You have opened a workspace
→ PROJECT CONTEXT — Voxy sees all the workspace's cards, the wiki, the history.
...
When no workspace is selected (you are on the **Home** tab):

- Voxy can create workspaces, list your existing workspaces, manage Home cards
- She can run web searches, execute system commands
- She schedules tasks (cron jobs, reminders)
- She doesn't "see" the inside of a specific workspace — if you ask her for info about a workspace, she has to open it first

**Good for:** Starting a new workspace, getting a high-level overview, tasks that aren't tied to a specific workspace.

### Workspace context — working on a workspace

When you are in a workspace's tab:

- Voxy sees all the workspace's cards (title, status, priority, assigned agent)
- She has access to the workspace's wiki and documents indexed in RAG
- She can create, edit, and move cards directly
- She can generate a standup, a brief, or a workspace health analysis
- The workspace description and context are injected into her system prompt

**Good for:** Managing workspace tasks, brainstorming, creating cards, getting a status report.
...
- The conversation in **workspace A** is not visible in **workspace B**
- The conversation on the **"Refactor auth" card** is not mixed with the workspace's conversation
- The **Home** tab has its own conversation, separate from any workspace
...
## Practical example: creating a DailyOps workspace

DailyOps is a typical use case: a workspace for managing recurring daily tasks — morning standup, weekly review, idea inbox. Here's how to set it up from start to finish.

---

### Step 1 — Create the workspace (general context)

From the **Home** tab, tell Voxy:

> "Create a workspace called DailyOps. It's for managing my recurring daily tasks — morning standup, weekly review, idea capture."

Voxy will create the workspace and take you there. You can also create it manually via the **+** button in the sidebar.

---

### Step 2 — Configure the workspace context (workspace context)

Once inside DailyOps, take a moment to describe the workspace to Voxy. This description is injected into her system prompt with every message:

> "Update the workspace description: DailyOps is my daily routine system. The goal is to have recurring tasks that regenerate automatically, and a space to capture random ideas without interrupting my flow."

You can also do this via the **workspace edit icon** in the interface.
...
From the workspace context, say:
...
Voxy will:
1. Look at the cards in progress in the workspace
...
An **Execute Board** automatically executes all cards in a workspace with a given status — no manual intervention needed.

> "Create an Execute Board job for the DailyOps workspace, every day at 8am, that executes cards with status 'todo'."
...
curl -X POST http://localhost:8000/api/jobs \
  ...
  -d '{
    ...
    "type": "execute_card",
    "schedule": "0 9 * * *",
    "payload": { "card_id": "TON_CARD_ID", "workspace_id": "TON_PROJECT_ID" }
  }'
```
...
### Step 6 — Use the workspace wiki

From the workspace context, the DailyOps wiki can serve as a permanent reference — review checklists, standup templates, context notes.
...
Voxy will create a card in the workspace's inbox (or in Home if specified) without pulling you out of your current context.
...
Voxy will generate a detailed description, acceptance criteria, and checklist suggestions based on the title and workspace context.

---

### Quick standup from workspace context

Without opening a card, from the workspace context:

> "Give me the workspace standup."
...
Each context (workspace or card) can have up to 5 parallel sessions — separate conversations on the same topic.
...
|-----------|----------------|------------------|
| Create a new workspace | General (Home) | "Create a workspace X" |
| See the status of all my workspaces | General | "Show me my workspaces" |
| Create a card | Workspace | "Create a card for Y" |
| See the standup | Workspace | "Workspace standup" |
