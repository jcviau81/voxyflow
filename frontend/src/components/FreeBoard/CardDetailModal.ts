/**
 * CardDetailModal — Detail modal for Main Board cards.
 * Shows title (editable), description (editable), color picker, mini-chat,
 * and actions (delete, assign to project, close).
 *
 * This replaces NoteDetailModal and works with Card objects instead of Ideas.
 */
...
export class CardDetailModal {
  ...
  constructor(private parentElement: HTMLElement) {
    ...
    // Refresh chat on any new message
  ...
  private async saveCard(updates: Partial<{ title: string; description: string; color: string | null }>): Promise<void> {
    if (!this.card) return;
  ...
  private renderContent(): void {
    ...
    if (this.card.color) this.modal.classList.add(`note-detail-modal--${this.card.color}`);
    ...
    titleEl.textContent = this.card.title;
    titleEl.addEventListener('blur', () => {
      const newTitle = (titleEl.textContent || '').trim();
      if (newTitle && newTitle !== this.card?.title) {
        this.saveCard({ title: newTitle });
      } else if (!newTitle) {
        titleEl.textContent = this.card?.title || '';
      }
    });
    titleEl.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'Escape') { titleEl.textContent = this.card?.title || ''; titleEl.blur(); }
    });
    ...
    contentTextarea.value = this.card.description || '';
    contentTextarea.addEventListener('blur', () => {
      const newDesc = contentTextarea.value;
      this.saveCard({ description: newDesc || '' });
    });

    const createdDate = new Date(this.card.createdAt).toLocaleDateString(undefined, {
    ...
    const chatHint   = createElement('div', { className: 'note-mini-chat-hint' }, 'scoped to this card');
    ...
    // Seed context on first open (no prior messages for this card session)
    const existingMessages = appState.getMessages(undefined, this.sessionId);
    if (existingMessages.length === 0) {
      const ctx = `Discussing card: ${this.card.title}\nDescription: ${this.card.description || '(empty)'}`;
