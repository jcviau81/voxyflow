...
export class ChatWindow {
  ...
  render(): void {
    ...
    // Always create VoiceInput — wake word mode should be accessible regardless of STT setting
    this.voiceInput = new VoiceInput(voiceContainer, sttBuiltinEnabled);
    if (!sttBuiltinEnabled) {
      // Hide the manual PTT mic button but keep wake word button visible
      // VoiceInput handles this internally — just don't hide the container
      voiceToggles.style.display = 'none'; // hide auto-send/auto-play toggles if STT disabled
    }
