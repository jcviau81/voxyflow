...
class PersonalityService:
    ...
    def _build_native_delegate_instructions(self) -> str:
        ...
        return (
            ...
            "`delegate_action` IMMEDIATELY, no questions, no plan, no « do you want me to…? »:\n"
            ...
            "→ call `delegate_action({\"action\":\"research\",\"summary\":\"Research gold-bearing rivers in Quebec\","
            "\"complexity\":\"complex\"})`\n"
            "Then reply: \"🚀 Worker launched on the research.\"\n\n"
            "User: \"do a web search on X\"\n"
            "→ call `delegate_action({\"action\":\"web_research\",\"summary\":\"Web search on X and summarize\"})`\n"
            "Then reply: \"🚀 Worker launched.\"\n\n"
            "User: \"work on the card / execute card Y\"\n"
            "→ call `delegate_action({\"action\":\"execute_card\",\"summary\":\"Execute card Y\","
            "\"card_title\":\"Y\",\"complexity\":\"complex\"})`\n"
            "Then reply: \"🚀 Worker launched on the card.\"\n\n"
            "User: \"write the code for Z\"\n"
            "→ call `delegate_action({\"action\":\"write_code\",\"summary\":\"Implement Z\","
            "\"complexity\":\"complex\"})`\n\n"
            "User: \"summarize this report\"\n"
            "→ call `delegate_action({\"action\":\"summarize\",\"summary\":\"Summarize the report\"})`\n\n"
            "**Anti-patterns to AVOID**:\n"
            "- ❌ \"Do you want me to launch a worker?\"           → just call delegate_action.\n"
            "- ❌ \"Here are the steps: 1. … 2. … 3. …\"          → just call delegate_action.\n"
            "- ❌ Long markdown explaining what you would do      → just call delegate_action.\n"
            "- ✅ One delegate_action call + one short confirmation line.\n\n"
            "Leave the `model` field unset — the runtime picks the worker tier from the\n"
            "action + complexity. Set `complexity:\"complex\"` for multi-step or destructive work,\n"
            "otherwise omit it.\n\n"
            "Reply to the user in their own language, but the rules above and the action\n"
            "verbs you call delegate_action with stay in English.\n"
            "Without delegate_action, nothing executes. Asking for confirmation = failure.\n\n"
    ...
    def _build_cli_mcp_delegate_instructions(self) -> str:
        ...
        return (
            ...
            "</delegate>\n"
            "`complexity` is an optional hint — `simple` for tiny one-shot tasks, `standard` is the "
            "default, `complex` for multi-step or destructive work. The runtime picks the actual "
            "worker model from your Worker Classes config (intent match) and the default worker model; "
            "don't try to name a specific model. Without <delegate>, complex tasks don't execute. "
    ...
    def _build_codex_mcp_delegate_instructions(self) -> str:
        ...
        return (
            ...
            "After consuming a result (memory/wiki/cards), call `workers.ack_artifact(task_id)`\n"
            "to close the loop and free disk. `workers.list_unread()` at session start shows\n"
