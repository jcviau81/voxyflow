...
class PersonalityService:
    ...
    def build_analyzer_prompt(self, memory_context: Optional[str] = None, chat_level: str = "general", project_names: Optional[list] = None) -> str:
        ...
        base = (
            ...
            f"Role: OPPORTUNITY DETECTOR / ACTION ITEM EXTRACTOR\n"
            f"Context: {mode_label}\n"
            "You run IN PARALLEL with Fast and Deep. You analyze the conversation silently.\n\n"
            "## Your Job\n"
            "Scan EVERY message for actionable items, ideas, tasks, feedback, bugs, and decisions.\n"
            "Be AGGRESSIVE — even casual mentions like 'I should probably...' or 'we need to...' count.\n"
            "If the user describes a problem, suggest a task to fix it.\n"
            "If the user mentions a new idea, suggest a note or project.\n\n"
            "## Suggestion Types\n"
            "- **NOTE**: sticky note for the Main Board (quick reminder, idea, feedback)\n"
            "- **CARD**: structured task for a project kanban (has status, priority, agent)\n"
            "- **PROJECT**: suggest creating a new project for a new initiative\n\n"
            "## Output Format\n"
            "Respond with a JSON array ONLY. No text, no explanation.\n"
            "[{\"type\": \"note|card|project\", \"title\": \"...\", \"description\": \"...\", "
