...
class PersonalityService:
    ...
    def _build_native_delegate_instructions(self) -> str:
        ...
        return (
            ...
            "Worker callbacks deliver only a short Haiku summary (kept brief on purpose). "
            "When you need the EXACT raw output a worker produced — file contents, command "
            "stdout, search results, logs — call `workers_read_artifact(task_id=...)`. "
            "Use offset/length to page through outputs larger than ~50k chars. The callback "
            "message advertises the artifact when one is available.\n\n"
    ...
    def _build_cli_mcp_delegate_instructions(self) -> str:
        ...
        return (
            ...
            "- `workers_read_artifact(task_id, offset?, length?)` — read the FULL verbatim "
            "output of a finished worker (file dumps, command stdout, logs). Worker callbacks "
            "only carry a short summary; call this when you need the exact raw content. "
            "Supports paging via offset/length.\n"
