...
|------|------|-----------|-------------|
| **prod** | TheThing | `thething` (100.96.26.98) | Live Voxyflow instance serving the dispatcher. Pull manually after merge to `main`. Reserved ports: 8000 (FastAPI), 18789 (Caddy). |
| **dev-primary** | ROG | `rog.tail6531d.ts.net` (100.67.12.87) | Auto-deploys `dev` branch every 5 min via systemd timer. RTX 3060, Caddy HTTPS reverse proxy, user-mode systemd. |
| **site host** | snaf.foo | `snaffoo` (100.80.12.91) | Hosts ONLY the static promotional site `voxyflow.snaf.foo` (rsync target from local `voxyflow-site` repo). Has nothing to do with Voxyflow runtime. |
...
- **`dev`** = active development. Auto-deploys to ROG. Open to direct push from maintainer.
- **`feature/*`** = feature branches. Workflow: branch from dev -> PR to dev -> validate on ROG -> PR dev -> main -> manual pull on TheThing.
...
### SSH access
```bash
ssh rog   # via stanza in ~/.ssh/config
# manual equivalent:
ssh -i ~/.ssh/id_ed25519_rog jcviau@rog.tail6531d.ts.net
```
Key `id_ed25519_rog` is a symlink -> `id_ed25519_ember` (same physical key, different intent label). It also authorises git push from ROG to `github.com/jcviau81`.
...
- All three: `systemctl --user enable`d on boot, `loginctl enable-linger jcviau` set so they survive logout.
...
4. SSH TheThing (prod) and: `cd ~/voxyflow && git pull origin main && systemctl restart voxyflow-backend` (or whatever the prod restart procedure is)

### Wipe + reinstall ROG (full reset)
See `docs/runbooks/rog-reinstall.md` (or this section as fallback):
...
8. Verify: `curl https://rog.tail6531d.ts.net/api/workspaces` -> 200 (NOT /health — that's currently a SPA catch-all, see backlog card 55156055)

### Demo environment
ROG is dev-exclusive. If a demo env is needed for a recording, spin up a separate temporary instance (e.g. Docker container or other Tailscale node). Do NOT cohabit demo with dev on ROG.
